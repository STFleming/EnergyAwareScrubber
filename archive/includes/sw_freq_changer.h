...
#define FMAP_SIZE 4096UL
#define FMAP_MASK (FMAP_SIZE - 1)
...
void setup_arm_pll( arm_pll_data * pll_data);
void set_fdiv_value( arm_pll_data * pll_data, int fdiv);
...
off_t cfg_reg = ARM_PLL_CFG;
off_t ctrl_reg = ARM_PLL_CTRL;
off_t status_reg = PLL_STATUS;
...
//-------------ARM_PLL_CTRL-----------------------------------------------------------------------
mapped_ctrl_reg = mmap(0, FMAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, memfd, ctrl_reg & ~FMAP_MASK);
...
pll_data->mapped_dev_ctrl_reg = mapped_ctrl_reg + (ctrl_reg & FMAP_MASK); //Our register is now at location mapped_dev_ctrl_reg in userspace
//---------------------------------------------------------------------------------------------

//-----------------ARM_PLL_CFG--------------------------------------------------------------------
mapped_cfg_reg = mmap(0, FMAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, memfd, cfg_reg & ~FMAP_MASK);
        if(mapped_cfg_reg == (void *) -1) {
                printf("Can't map the memory to user space.\n");
                exit(0);
        }
//printf("Memory mapped at address %p. \n", mapped_cfg_reg);
pll_data->mapped_dev_cfg_reg = mapped_cfg_reg + (cfg_reg & FMAP_MASK);
//------------------------------------------------------------------------------------------------

//---------------PLL_STATUS-----------------------------------------------------------------------
mapped_status_reg = mmap(0, FMAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, memfd, status_reg & ~FMAP_MASK);
        if(mapped_status_reg == (void *) -1) {
                printf("Can't map the memory to user space. \n");
                exit(0);
        }
//printf("Memory mapped at address %p. \n", mapped_status_reg);
pll_data->mapped_dev_status_reg = mapped_status_reg + (status_reg & FMAP_MASK);
//------------------------------------------------------------------------------------------------

}

void set_fdiv_value( arm_pll_data * pll_data, int input_divider)
{
...
while( CHECK_BIT(*((volatile unsigned long *)pll_data->mapped_dev_status_reg),0) ) {  }
//------------------------------------------------------------------------------
